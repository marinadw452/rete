...
CREATE INDEX idx_available_captains ON users (role, is_available, city, neighborhood)
...
CREATE INDEX idx_users_location ON users (city, neighborhood);

-- فهرس المطابقات حسب التاريخ
CREATE INDEX idx_matches_date ON matches (created_at DESC);

-- فهرس التقييمات
CREATE INDEX idx_ratings_user ON ratings (rated_user, rating);
